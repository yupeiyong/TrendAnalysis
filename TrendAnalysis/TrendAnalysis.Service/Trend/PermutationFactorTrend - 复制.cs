...
namespace TrendAnalysis.Service.Trend
{
    ...
    ///     排列因子的历史趋势
    /// </summary>
    public class PermutationFactorTrend_Old
    {
        ...
        /// <param name="dto">记录集合、比较因子、允许的最小连续次数，大于等于此数才记录......</param>
        /// <returns></returns>
        public List<PermutationFactorTrendConsecutiveDetails<T>> AnalysepredictiveFactorAtFirst<T>(PermutationFactorTrendAnalyseDto<T> dto)
        {
            //统计连续次数
            var factorResults = CountConsecutives(dto.Numbers, dto.PermutationFactors, dto.NumbersTailCutCount, dto.AllowMinTimes);
            factorResults = factorResults.Where(t => t.HistoricalConsecutiveTimes.Count > 0).ToList();
        ...
        /// <returns></returns>
        public static List<PermutationFactorTrendConsecutiveDetails<T>> CountConsecutives<T>(List<T> numbers, List<List<Factor<T>>> permutationFactors, int numbersTailCutCount = 1, int allowMinTimes = 1)
        {
            #region 因子排列
            ...
            var resultList = new List<PermutationFactorTrendConsecutiveDetails<T>>();
